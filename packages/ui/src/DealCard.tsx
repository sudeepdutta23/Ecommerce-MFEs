 ...
 */
export function DealCard({ name, price, mrp, currency = 'INR', media, footer, className }: DealCardProps) {
  ...
  return (
    <div
      className={cn(
        'relative flex h-full flex-col overflow-hidden rounded-card border border-slate-200 bg-surface shadow-card transition-shadow hover:shadow-card-hover',
        className,
      )}
    >
      {hasDiscount ? (
        <span className="absolute right-0 top-0 z-10 rounded-bl-xl bg-brand-500 px-2.5 py-1.5 text-center text-xs font-semibold leading-tight text-white">
          {discountPercent}%
          <br />
          OFF
        </span>
      ...
      <div className="flex h-36 items-center justify-center bg-surface-sunken text-6xl">
        {media}
      </div>
